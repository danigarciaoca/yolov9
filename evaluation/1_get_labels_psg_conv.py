...
def process_image(det_model, in_img_path, out_labels_path, view_img):
    ...
    det = det_model(img, classes=None, conf_thres=0.1, max_det=1000)[0]  # single image, so just keep item 0

    # annotate image with detections
    txt_out = out_labels_path / in_img_path.stem
    img_out = model.annotate(img, det, line_width=3, annot_img=view_img,
                             save_txt=True, save_conf=True, txt_file=txt_out)
...
if __name__ == "__main__":
    ...
    labels_path = Path("coco_test_yolov9_psg_conv/labels/test")
    labels_path.mkdir(parents=True, exist_ok=True)

    # load model
    weights_path = "/home/danigarciaoca/PycharmProjects/yolov9/yolov9/src/runs/train/exp/weights/best-converted.pt"  # model weights
